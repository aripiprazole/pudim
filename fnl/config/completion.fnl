...
;; Setup treesitter
(nvim-treesitter.setup {:ensure_installed all :highlight {:enable true}})
...
(fn t [string]
  (vim.api.nvim_replace_termcodes string true true true))

(fn check-backspace []
  (let [col (- (vim.fn.col ".") 1)]
    (if (= col 0) true (let [l (vim.fn.getline ".")
                             s (l:sub col col)]
                         (not= (s:match "%s") nil)))))
...
(set _G.tab_complete (fn []
                       (if (= (vim.fn.pumvisible) 1) (t :<C-n>)
                           (= (vim.fn.vsnip#available 1) 1) (t "<Plug>(vsnip-expand-or-jump)")
                           (check-backspace) (t :<Tab>)
                           (vim.fn.compe#complete))))

(set _G.s_tab_complete (fn []
                         (if (= (vim.fn.pumvisible) 1) (t :<C-p>)
                             (= (vim.fn.vsnip#jumpable -1) 1) (t "<Plug>(vsnip-jump-prev)")
                             (t :<S-Tab>))))
