...
(fn on-atach [client buf]
  ...
  (buf-set-keymap buf :i :<C-Space> "compe#complete()"
                  {:silent true :expr true})
  (buf-set-keymap buf :i :<CR> "compe#confirm('<CR>')"
                  {:silent true :expr true})
  (if client.resolved_capabilities.document_formatting
      (buf-set-keymap buf :n :<leader>lf
...
(each [_ value (ipairs servers)]
  (let [config (. lspconfig value)]
    (config.setup {: capabilities :on_attach on-atach})))
...
(set vim.lsp.handlers.textDocument/typeDefinition
     locations.typeDefinition_handler)
(set vim.lsp.handlers.textDocument/implementation
...
(vim.api.nvim_set_keymap :n :<C-p>
                         ...
                         {:noremap true :silent true})
(vim.api.nvim_set_keymap :n :rn "<cmd>Lspsaga rename<CR>"
                         {:noremap true :silent true})
